...
void update(ll node,ll l, ll r,ll i,ll j,ll value)
{
    ...
    tree[node].cost=tree[node*2].cost+tree[node*2+1].cost+((r-l+1)*tree[node].prop);
}

ll query(ll node,ll l,ll r,ll i,ll j,ll carry)
{
    if(l>j or r<i)
    {
        return 0;
    ...
    if(l>=i and r<=j)
    {
        return tree[node].cost+((r-l+1)*carry);
    }


    ll mid=(l+r)/2;
    ll xx=query(node*2,l,mid,i,j,carry+tree[node].prop);
    ll yy= query(node*2+1,mid+1,r,i,j,carry+tree[node].prop);
